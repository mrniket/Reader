...
-(long)insertRule:(NSString *)rule index:(unsigned long)index
{
	...
	{
		SVGKCSSStyleRule* newRule = [[SVGKCSSStyleRule alloc] initWithSelectorText:[stringSplitContainer[0] substringFromIndex:1] styleText:stringSplitContainer[1]];
...
- (instancetype)initWithString:(NSString*) styleSheetBody
{
    ...
    if (self)
	{
		self.cssRules = [[SVGKCSSRuleList alloc]init];
