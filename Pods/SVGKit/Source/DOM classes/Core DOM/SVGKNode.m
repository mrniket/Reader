...
- (instancetype)initType:(DOMNodeType) nt name:(NSString*) n value:(NSString*) v
{
    ...
    if (self) {
		...
		self.childNodes = [[SVGKNodeList alloc] init];
    }
    return self;
}
    ...
    if (self) {
        ...
        switch( nt )
		{
			...
			case DOMNodeType_ELEMENT_NODE:
			{
				...
				self.attributes = [[SVGKNamedNodeMap alloc] init];
			}break;
		}
...
//	NSAssert(namespaceShortnames != nil, @"Must supply an empty dictionary for me to fill with encountered namespaces, and the shortnames I invented for them!");
	...
	{
		SVGKNode* attribute = xmlnsNodemap[xmlnsNodeName];
	...
	for( NSString* namespace in nodeMapsByNamespace )
	{
		...
		{
			SVGKNode* attribute = nodeMap[nodeNameFromMap];
